...
class V1GameState:
    ...
    def _recalculate_fields(self):
        ...
        for player_data in self.players:
            spawn_id = player_data.spawn_id
            player_data.update_from_v2(
                self._game_state.cars[spawn_id],
                spawn_id_spectator_id_map[spawn_id],
                self._boost_pickups[spawn_id],
                self._car_ball_touched[spawn_id],
            )
    ...
    def update(self, packet: GamePacket, extra_info: Optional[ExtraPacketInfo] = None):
        ...
        for player_info in packet.players:
            spawn_id = player_info.spawn_id
            if spawn_id not in self._boost_pickups:
                self._boost_pickups[spawn_id] = 0
            ...
            if player_info.latest_touch is not None:
                ...
                if ticks_since_touch < self._tick_skip:
                    self._car_ball_touched[spawn_id] = True
            if (
